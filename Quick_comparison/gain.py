...
def loop(dataset,sep=',',na_values='?',outcome_Type='binaryClass',problem='C',vmaps={},parameter={}):
    ...
    # Impute missing data
    imputed_data_x = gain(x.values, gain_parameters)
...
def gain (data_x, gain_parameters):
  ...
  ## GAIN loss
  D_loss_temp = -tf.reduce_mean(M * tf.compat.v1.log(D_prob + 1e-8) \
                                + (1-M) * tf.compat.v1.log(1. - D_prob + 1e-8)) 
  
  G_loss_temp = -tf.reduce_mean((1-M) * tf.compat.v1.log(D_prob + 1e-8))
...
for file_name in ['realdata\\water-treatment.csv']: 
    ...
    #error,total =
    print('Loop')
    loop(dataset=file_name,sep=';',na_values='?',outcome_Type='binaryClass',problem='C',vmaps=vmaps,parameter={})
                #print('Error for ' + file_name+   ' : ' + str(error) + '  Time : ' + str(datetime.timedelta(seconds=total)) + ' Params  : ' + str({}) )
