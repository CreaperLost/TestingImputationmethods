...
for ep in [500,1000]:
        for hi_p in [0.5,0.9]:
            for alp in [1,10]:
                error , total = loop(dataset='realdata\lymphoma_2classes.csv',sep=';',na_values='?',outcome_Type='binaryClass',problem='C',vmaps={},parameter={"iterations":ep,"hint_rate":hi_p,"batch_size":16,"alpha":alp,"Binary_Indicator":True})
                print('Error for ' + 'realdata\lymphoma2_classes.csv'+   ' : ' + str(error) + '  Time : ' + str(datetime.timedelta(seconds=total)) + ' Params  : ' + str({"epochs":ep,"hint_rate":hi_p,"alpha":alp}) )
                with open('gain_res+bi.txt', 'a') as the_file:
                    the_file.write('Error for ' + 'realdata\lymphoma_2classes.csv'+   ' : ' + str(error) + '  Time : ' + str(datetime.timedelta(seconds=total)) + ' Params  : ' + str({"epochs":ep,"hint_rate":hi_p,"alpha":alp}) + '\n' )
"""
