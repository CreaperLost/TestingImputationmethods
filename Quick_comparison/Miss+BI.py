...
def loop(dataset,sep=',',na_values='?',outcome_Type='binaryClass',problem='C',vmaps={},parameters={}):
    ...
    for train_index, test_index in kf.split(x):
        ...
        start = time.time()
        imputer = IterativeImputer(parameters=parameters,names=column_names,vmaps=vmaps) 
...
for file_name in glob.glob('realdata/'+'*.csv'):
    ...
    error,total=loop(dataset=file_name,sep=';',na_values='?',outcome_Type='binaryClass',problem='C',vmaps=vmaps,parameters = {"Binary_Indicator":True})
    print('Error for ' + file_name+   ' : ' + str(error) + '  Time : ' + str(datetime.timedelta(seconds=total)) )
    with open('miss+bi.txt', 'a') as the_file:
